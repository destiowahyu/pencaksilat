...
try {
    ...
    ");
    $stmt->execute([$user_id]);
    $admin_stats['total_competitions'] = $stmt->fetch()['total'];
    ...
    ");
    $stmt->execute([$user_id]);
    $admin_stats['active_competitions'] = $stmt->fetch()['total'];

    // Get total registrations for admin's competitions
    $stmt = $pdo->prepare("
        SELECT COUNT(*) as total 
        FROM registrations r 
        JOIN competition_admins ca ON r.competition_id = ca.competition_id 
        WHERE ca.admin_id = ?
    ");
    $stmt->execute([$user_id]);
    $admin_stats['total_registrations'] = $stmt->fetch()['total'];
    ...
    $stmt = $pdo->prepare("
        ...
        FROM registrations r 
        JOIN competition_admins ca ON r.competition_id = ca.competition_id 
        WHERE ca.admin_id = ? AND r.payment_status = 'unpaid'
    ");
    $stmt->execute([$user_id]);
    $admin_stats['pending_payments'] = $stmt->fetch()['total'];

} catch (PDOException $e) {
    // Handle errors silently
}

// Get managed competitions list
...
try {
    ...
    ");
    $stmt->execute([$user_id]);
    $managed_competitions = $stmt->fetchAll();
} catch (PDOException $e) {
    $managed_competitions = [];
}
?>
...
<head>
    ...
    <title>Akun Saya - Admin Panel</title>
...
<body>
    ...
    <div class="main-content">
        <div class="page-header">
            ...
            <p class="page-subtitle">Kelola informasi profil dan akun Administrator</p>
        </div>
